...
import { Receipt, Download, CreditCard, ShieldCheck, Printer, CheckCircle2, FileText } from 'lucide-react';
  ...
  return (
    <div className="flex-1 pb-16">
      <TopHeader
        ...
        subtitle="Complete transparent record of recurring membership contributions and Dakshina settlements."
      />
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 py-8 space-y-8">
        <div className="bg-warmwhite rounded-3xl p-6 sm:p-8 border border-sand shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-cinzel text-lg font-bold text-charcoal-900">Payment History ({payments.length})</h3>
            <div className="flex items-center space-x-2 text-xs text-charcoal-800/70">
              <ShieldCheck className="w-4 h-4 text-emerald-600" />
              <span>100% Secure Server-Side Verified</span>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-sand font-medium text-charcoal-900">
                {payments.map((p) => (
                  <tr key={p._id} className="hover:bg-cream/30 transition-colors">
                    ...
                    <td className="py-4 px-4 text-charcoal-800/70">{new Date(p.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                    <td className="py-4 px-4">Sradham 360 Monthly Subscription</td>
                    <td className="py-4 px-4 uppercase font-mono text-[11px]">{p.provider}</td>
                    <td className="py-4 px-4 font-bold text-sm">₹{p.amount}</td>
          ...
          <div className="space-y-6 text-xs text-charcoal-900">
            ...
            <div id="printable-receipt" className="p-6 sm:p-8 rounded-2xl bg-white border border-sand shadow-inner space-y-6 font-sans">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-canvas border border-sand text-[11px]">
                <div>
                  <span className="text-[9px] font-bold uppercase text-charcoal-800/60 block">Billed To (Customer)</span>
                  <p className="font-bold text-charcoal-900 mt-0.5">Sundaram Sharma</p>
                  <p className="text-charcoal-800/70">Flat 4B, Heritage Towers, Mylapore, Chennai</p>
                  <p className="text-charcoal-800/60 font-mono text-[10px]">+91 94440 98765</p>
                </div>

                <div>
                  <span className="text-[9px] font-bold uppercase text-charcoal-800/60 block">Payment Method & Reference</span>
                  ...
                  <p className="text-emerald-700 font-semibold text-[10px]">Status: Verified & Settled</p>
                </div>
              </div>
                ...
                <tbody className="divide-y divide-sand/60 text-[11px]">
                  <tr>
                    <td className="py-2.5 font-medium">
                      Standard Sradham 360 Recurring Membership
                      <span className="block text-[10px] text-charcoal-800/60">
                        Includes complete annual ritual coordination, 4-member Vedic team allocation & samagri.
                      </span>
                    </td>
